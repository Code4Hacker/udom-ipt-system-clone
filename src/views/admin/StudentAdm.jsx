...
const StudentAdm = () => {
  ...
  const onRowSelect = (event) => {
    event.data.path_url !== "No Upload"?
    window.location  = `${baseURL}${event.data.path_url}`:
    toast.error(`We can't direct this url -> ${event.data.path_url}`);
    jQuery("td").css({
  ...
  const cols = [
    { field: 'sn', header: '#' },
    { field: 'fullname', header: 'Student Name' },
    { field: 't_number', header: 'Registration Number' },
    { field: 'mobile', header: 'Contact' },
    { field: 'gender', header: 'Gender' },
    { field: 'email', header: 'Email' },
    { field: 'path_url', header: 'Uploads' },
  ];
//   const exportColumns = cols.map((col) => ({ title: col.header, dataKey: col.field }));
  ...
  const getModulesDetails = async () => {
    try {
      const requests = axios.request({
        method: "POST",
        url: `${baseURL}students_and_arrival.php`
      });
      (await requests).data.status === 200 ?
    setProject((await requests).data.students): toast.error("No Data found")
    } catch (error) {
      toast.error(`Something went wrong\n${error}`);
    }
  }
